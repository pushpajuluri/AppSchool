...
func callToGetToday(successblock : @escaping (AnyObject?) -> Void,FailureBlock failureblock : ((NSError?) -> Void)?,viewController:UIViewController){
    ...
    let url = URL(string: "\(Constants.APP_URL)/1/teacherschedule/today")
    var request = URLRequest(url: url!)
    request.httpMethod = "POST"
    request.addValue("application/json", forHTTPHeaderField: "Content-Type")
    request.addValue("application/json", forHTTPHeaderField: "Accept")
    ...
    NSURLConnection.sendAsynchronousRequest(request, queue: OperationQueue.main) { (response, data, error) in
        ...
        func callToGetMysubjects(successblock : @escaping (AnyObject?) -> Void,FailureBlock failureblock : ((NSError?) -> Void)?,viewController:UIViewController){
            ...
            let url = URL(string: "\(Constants.APP_URL)/1/mysubjects")
            var request = URLRequest(url: url!)
            request.httpMethod = "POST"
            request.addValue("application/json", forHTTPHeaderField: "Content-Type")
            request.addValue("application/json", forHTTPHeaderField: "Accept")
    ...
    func callToGetMybirthday(successblock : @escaping (AnyObject?) -> Void,FailureBlock failureblock : ((NSError?) -> Void)?,viewController:UIViewController){
        let postString = ["id":self.id,"sectionname":self.sectionname] as [String : Any]
        let url = URL(string: "\(Constants.APP_URL)/1/birthday/students")
        var request = URLRequest(url: url!)
        request.httpMethod = "POST"
        request.addValue("application/json", forHTTPHeaderField: "Content-Type")
        request.addValue("application/json", forHTTPHeaderField: "Accept")
        ...
        NSURLConnection.sendAsynchronousRequest(request, queue: OperationQueue.main) { (response, data, error) in
            ...
            do {
                ...
                DispatchQueue.main.async {
                    successblock(todayBirthday as AnyObject?)
                }
            }
            catch {
            }
